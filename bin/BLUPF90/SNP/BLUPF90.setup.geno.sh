...
#SBATCH --job-name=Setup.BLUPF90.SNP
#SBATCH --output=Setup.BLUPF90.SNP_%j.out
#SBATCH --error=Setup.BLUPF90.SNP_%j.err
...
SNPGeno=${data}/Genotypes/SNP/Seminole.Nutritional
newname=Seminole.Nutri.SNP
phenotxt=${data}/Phenotypes/
AIREML=${my_bin}/BLUPF90/AIREML.parameter
GBLUP=${my_bin}/BLUPF90/GBLUP.parameter
cd ${results}
mkdir SNP.BLUPF90
cd ${results}/SNP.BLUPF90
mkdir Genotypes
cd ${results}/SNP.BLUPF90/Genotypes
ml plink
for TRAIT in $(echo "MARB" "HCW")
do
    cd ${results}/SNP.BLUPF90/Genotypes
    mkdir ${TRAIT}
    cd ${results}/SNP.BLUPF90/Genotypes/${TRAIT}
    ...
    sed -e "s/.add/.dom_dev/g" AIREML.parameter.add > AIREML.parameter.dom_dev
    rm AIREML.parameter
    cp ${GBLUP} .
    sed -e "s/#YOUR_TRAITS/${TRAITS_HEAD}/g" \
    -e "s/YOUR_PED/${newname}.PED.add/g" \
    ...
    sed -e "s/.add/.dom_dev/g" GBLUP.parameter.add > GBLUP.parameter.dom_dev
    rm GBLUP.parameter
    cd ${results}/SNP.BLUPF90/Genotypes
done
...
do
    ...
    cp ${SNPGeno}.fam FAM.file
    grep -wf id.temp FAM.file | awk '{print $1,$2}' > id2.temp
    # keep 
    plink -cow -bfile ${SNPGeno} -chr 1-29  -keep id2.temp -maf 0.01 -geno 0.1 -mind 0.1 -hwe 1e-100 keep-fewhet -make-bed --keep-allele-order --out data.temp
    plink -cow -bfile data.temp --recode A  -make-bed --keep-allele-order --out ${newname}
    ...
    cut  -f7- ${newname}.raw | sed -e '1d' -e 's/NA/5/g'  -e 's/2/0/g' -e 's/[[:blank:]]//g' > geno.temp.dom_dev
    # count number of characters in each line. which should be your number of markers in this case 221115
    awk '{ print length }'  geno.temp.add |uniq
    ...
    paste Ids2.temp geno.temp.dom_dev -d " " > ${newname}.geno.dom_dev
    ...
    awk -NF " " '{print $1,"0","0"}' ${newname}.geno.dom_dev > ${newname}.PED.dom_dev
